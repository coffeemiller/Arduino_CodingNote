...
---------------------------------------------------------
## 제19강.Final 키워드
+ 자바에서는 절대 변하지 않는 특정한 것을 정하고 싶을 때는 최종(final)을 사용한다. 이 키워드는 변수, 메소드 클래스에 모두 사용할 수 있다. 변수에 사용할 경우 변하지 않는 상수가 되며, 메소드가 사용할 때는 제정의가 불가능한 메소드가 되어, 클래스에 사용할 때는 상속이 불가능한 하나의 완전한 클래스가 되는 것이다.

```java
//Parent 클래스
public class Parent {
	public final void show() {   // final 키워드를 붙여 영원한 상수.
...
+ 인터페이스는 숙련된 자바 개발자들에게 아주 선호되는 설계 기능이면서 자바에서 다중 상속을 구현하게 해주는 고급 기술이기도 하다.
+ 추상클래스는 추상 메소드 외에 멤버변수나 일반 메소드를 가질 수 있지만 인터페이스에서는 반드시 사전에 정의된 추상 메소드와 상수만을 가질 수 있다는 특징이 있다.
